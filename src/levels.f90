subroutine dynprf (No_Rows)
  ...
  integer :: P_height(52)
  integer :: Wind_dir(52)
  integer(kind=1) :: I_Header
          
!      INCLUDE 'modvars.h'

  data I_Header,No_Levels, Thick_Level, No_variables /1, 52, 50, 4/
...
!      first wind speed and direction
!      now temperature and humidity
    q_profile(L+1) = q_fine(L) * 1000   !g/Kg
    t_profile(L+1) = t_fine(L) - 273.13 !C
    P_height(l+1) = 50*L
  end do

! now fill in first elements of array
! wind speed is uten and direction same as at 50 m
! wind speed at lowest layer not calculated at night
